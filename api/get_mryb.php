...
if ($k != "")
	$sql = "SELECT a.id, a.title, b.image, a.vdesc, a.vkey, a.len, a.oripv, a.pv, a.dateline FROM cmsnew.video_list a, cmsnew.base_publish b where a.id = b.target_id and a.id in (select target_id from cmsnew.base_taglist where tag_id = 100) and (b.related_class_id is null or b.related_class_id = 1)  and a.isdel = 0 and a.valid = 1 and a.title like '%$k%' order by a.dateline desc limit $pageno,$pagesize";
else
