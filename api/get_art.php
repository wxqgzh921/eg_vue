...
if ($k != "")
	$sql = "select `a`.`target_id` as `id`, `a`.`title`, `a`.`image`, `a`.`indexcontent`, `a`.`starttime`  from `base_publish` `a` where `a`.`valid`=1  and `a`.`target_type`=2  and `a`.`pic`=1 and `a`.`title` like '%$k%' ORDER BY `a`.`starttime` DESC limit $pageno,$pagesize";
else
	$sql = "select `a`.`target_id` as `id`, `a`.`title`, `a`.`image`, `a`.`indexcontent`, `a`.`starttime`  from `base_publish` `a` where `a`.`valid`=1  and `a`.`target_type`=2  and `a`.`pic`=1  ORDER BY `a`.`starttime` DESC limit $pageno,$pagesize";
