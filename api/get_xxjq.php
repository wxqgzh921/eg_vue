...
if ($k != "")
	$sql = "select a.id, a.title, b.image, a.vdesc, a.vkey, a.len, a.oripv, a.pv, a.dateline from video_list a, base_publish b,video_albumvideo c where a.id = b.target_id and b.target_id=c.vid and b.target_type=1 and c.album_id=887 and c.valid=1 and b.valid=1 and a.title like '%$k%' order by a.dateline desc limit $pageno,$pagesize";
else
	$sql = "select a.id, a.title, b.image, a.vdesc, a.vkey, a.len, a.oripv, a.pv, a.dateline from video_list a, base_publish b,video_albumvideo c where a.id = b.target_id and b.target_id=c.vid and b.target_type=1 and c.album_id=887 and c.valid=1 and b.valid=1 order by c.displayorder desc,c.addtime desc limit $pageno,$pagesize";
